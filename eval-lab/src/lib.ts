...
export function readStars(): Star[] {
  return Schema.decodeUnknownSync(StarsFile)(
    readFileSync(path.join(DATA_DIR, "stars.json"), "utf8"),
  );
}
...
export function filterSql(f: Filters): FilterSql {
  ...
  if (f.language) {
    if (f.language.toLowerCase() === "unknown") clauses.push("language IS NULL");
      ...
      clauses.push("LOWER(language) = LOWER(?)");
      params.push(canonicalLanguage(f.language));
    }
  }

  if (f.minStars !== undefined) {
    clauses.push("stars >= ?");
    params.push(f.minStars);
  }

  if (f.maxStars !== undefined) {
    clauses.push("stars <= ?");
    params.push(f.maxStars);
  }
...
export function computeBoosts(star: Star, query: string, nowMs = Date.now()): BoostResult {
  ...
  if (!Number.isNaN(starredMs)) {
    ...
    if (days <= 90) {
      factor *= 1.1;
      reasons.push("starred<90d ×1.10");
    } else if (days <= 365) {
      factor *= 1.05;
      reasons.push("starred<1y ×1.05");
    }
  }
...
export function rrf(
  legs: (Ranked[] | null)[],
  k = 60,
): Map<number, { rrf: number; legs: (number | null)[] }> {
