...
interface QueryResult {
  ...
  modes: Record<string, { metrics: Metrics; hits: Hit[]; timing_ms: Record<string, number>; expansion: string[]; fallback: string | null; legs: SearchOutput['legs'] }>;
  orphanedGold: string[];
}
...
function computeMetrics(hits: Hit[], rel: Record<string, number>): Metrics {
  ...
  const g2 = Object.entries(rel).filter(([, g]) => g === 2).map(([n]) => n);
  ...
  const r10 = g2.length === 0 ? null : g2.filter((n) => top.some((h) => h.full_name === n)).length / g2.length;
  let mrr: number | null = null;

  for (let i = 0; i < top.length; i++) {
    if (grade(top[i].full_name) >= 1) { mrr = 1 / (i + 1); break; }
  }

  const dcg = top.reduce((a, h, i) => a + (2 ** grade(h.full_name) - 1) / Math.log2(i + 2), 0);

  const idcg = Object.values(rel).sort((a, b) => b - a).slice(0, 10)
...
function printQueryResult(qr: QueryResult): void {
  ...
  if (qr.orphanedGold.length) console.log(`   ⚠ gold repos not in corpus: ${qr.orphanedGold.join(', ')}`);
  console.log('');
  ...
  for (const [label, get] of rows) {
    console.log([label.padEnd(12), ...MODES.map((m) => fmt(get(qr.modes[m].metrics)).padStart(14))].join(''));
  }

  const legInfo = MODES.map((m) => `${m}:kw=${qr.modes[m].legs.keyword},sem=${qr.modes[m].legs.semantic}${qr.modes[m].legs.filtered_universe !== null ? `,uni=${qr.modes[m].legs.filtered_universe}` : ''}`).join('  ');
  console.log(`leg sizes: ${legInfo}`);
  console.log('');
  const w = 44;
  console.log(['rank', ...MODES.map((m) => m.padEnd(w))].map((s, i) => (i === 0 ? s.padEnd(5) : s)).join(''));
...
async function main(): Promise<void> {
  const golden = Schema.decodeUnknownSync(GoldQueriesFile)(readFileSync(path.join(LAB_DIR, 'gold', 'queries.json'), 'utf8'));
  const index = LabIndex.open();
  console.log(`# starwatch eval-lab — search quality run`);
  console.log(`corpus: ${index.repoCount} repos · gold queries: ${golden.queries.length} · modes: ${MODES.join(', ')} · boost factors: ON`);
  ...
  for (const q of golden.queries) {
    ...
    for (const mode of MODES) {
      const out = await runSearch(index, { query: q.query, mode, filters: q.filters, limit: 10, boost: true });
      modes[mode] = {
  ...
  console.log('');
  console.log(['metric', ...MODES].map((m, i) => (i === 0 ? m.padEnd(12) : m.padStart(14))).join(''));
  ...
  console.log(['class', ...MODES].map((m, i) => (i === 0 ? m.padEnd(14) : m.padStart(14))).join(''));

  for (const c of classes) {
    const rs = results.filter((r) => r.class === c);
    console.log([c.padEnd(14), ...MODES.map((m) => fmt(avg(rs.map((r) => r.modes[m].metrics.ndcg10))).padStart(14))].join(''));
  }
  ...
  const payload = {
    ...
    modes: MODES,
    queries: results,
    overall: Object.fromEntries(MODES.map((m) => [m, Object.fromEntries(rows.map(([label, get]) => [label, avg(results.map((qr) => get(qr, m)))]))])),
  };
