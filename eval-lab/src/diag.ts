...
function metrics(hits: Hit[], rel: Record<string, number>) {
  ...
  const r10 = g2.length ? g2.filter((n) => top.some((h) => h.full_name === n)).length / g2.length : null;
  let mrr: number | null = null;

  for (let i = 0; i < top.length; i++) if (grade(top[i].full_name) >= 1) { mrr = 1 / (i + 1); break; }

  const dcg = top.reduce((a, h, i) => a + (2 ** grade(h.full_name) - 1) / Math.log2(i + 2), 0);
  const idcg = Object.values(rel).sort((a, b) => b - a).slice(0, 10).reduce((a, g, i) => a + (2 ** g - 1) / Math.log2(i + 2), 0);
  ...
  for (const q of golden.queries) {
    for (const mode of MODES) {
      for (const boost of [true, false]) {
        const out = await runSearch(index, { query: q.query, mode, filters: q.filters, limit: 10, boost });
        ...
        if (q.filters.language) {
          ...
          if (bad.length > 0) console.log(`  !! filter violation on ${q.id}: ${bad.map((b) => b.full_name).join(',')}`);
        }

        const m = metrics(out.hits, q.relevance);
        console.log(`${q.id.padEnd(21)}${mode.padEnd(14)}${String(boost).padEnd(8)}${f(m.p5g2).padEnd(7)}${f(m.r10).padEnd(6)}${f(m.mrr).padEnd(6)}${f(m.ndcg)}`);
      }
    }
  }
}
