...
import {
  ...
  openDb, EMBED_DIMS, type Filters, type Ranked, type RepoRow,
} from './lib.ts';
...
export class LabIndex {
  ...
  constructor(db: DatabaseSync) {
    ...
    for (const r of db.prepare('SELECT * FROM repos').all()) {
      this.rows.set((r as { id: number }).id, r as unknown as RepoRow);
    }
  }
  ...
  loadVectors(): Map<number, Float32Array> {
    ...
    for (const r of this.db.prepare('SELECT repo_id, vector FROM embeddings').all()) {
      const { repo_id, vector } = r as { repo_id: number; vector: Uint8Array };
      const bytes = vector.slice();
      map.set(repo_id, new Float32Array(bytes.buffer, bytes.byteOffset, bytes.byteLength / 4));
    }
    this.vectors = map;
    return map;
  }
  ...
  keyword(terms: string[], filterIds: Set<number> | null, useExpansion: boolean): {
    list: Ranked[]; fallback: string | null; primaryCount: number;
  } {
    ...
    const run = (table: 'repos_fts' | 'repos_tri', match: string): { id: number; score: number }[] => {
      ...
      return this.db.prepare(sql).all(match) as { id: number; score: number }[];
    };
    const rank = (rows: { id: number; score: number }[]): Ranked[] =>
...
export async function runSearch(index: LabIndex, opts: SearchOptions): Promise<SearchOutput> {
  ...
  let candidates = [...fused.entries()].map(([id, v]) => ({
    id,
    base: v.rrf,
    lex_rank: v.legs[0],
    sem_rank: v.legs[1],
