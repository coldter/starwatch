...
import { RepoStore } from "../../src/storage/repos.ts";import { applyMigration, makeIndexState, makeRepo, makeUser, makeSqliteLayer } from "./support.ts";

const LOGIN = "coldter";

const testLive = () => RepoStore.layer.pipe(Layer.provideMerge(makeSqliteLayer()));

const ids = (repos: ReadonlyArray<{ readonly id: number }>): ReadonlyArray<number> => repos.map((repo) => repo.id);
...
describe("RepoStore", () => {
  ...
  it.effect("upserts and reads index state", () =>
    Effect.gen(function* () {
      ...
      const state = makeIndexState(LOGIN, { phase: "listing", starsTotal: 3448 });
      yield* store.upsertIndexState(state);
      expect(yield* store.getIndexState(LOGIN)).toEqual(state);

      yield* store.upsertIndexState({ ...state, phase: "failed", lastError: "boom" });
  ...
  it.effect("upserts repos idempotently and preserves the original starred_at", () =>
    Effect.gen(function* () {
      ...
      yield* store.upsertRepos(LOGIN, [
        makeRepo(1, "owner/one", { stars: 1, starredAt: "2026-01-01T00:00:00Z" })
      ]);
      // A later re-list may omit starredAt; the stored value must win.
      yield* store.upsertRepos(LOGIN, [makeRepo(1, "owner/one", { stars: 2, description: "updated" })]);
  ...
  it.effect("chunks reads beyond the 100 bound-parameter limit", () =>
    Effect.gen(function* () {
      ...
      expect((yield* store.getRepos(LOGIN, repos.map((repo) => repo.id))).length).toBe(120);
    }).pipe(Effect.provide(testLive()))
  );
  ...
  it.effect("filters topics with AND semantics and stars by range", () =>
    Effect.gen(function* () {
      ...
      expect(ids(yield* store.listReposForSearch(LOGIN, { topics: ["http", "client"] }))).toEqual([4, 1]);
      expect(ids(yield* store.listReposForSearch(LOGIN, { topics: ["http", "client", "retry"] }))).toEqual([4]);
      ...
      expect(ids(yield* store.listReposForSearch(LOGIN, { minStars: 50, maxStars: 1000 }))).toEqual([4, 1, 3]);
  ...
  it.effect("filters by group slugs with OR semantics", () =>
    Effect.gen(function* () {
      ...
      yield* store.replaceGroups(LOGIN, [
        { id: "UL_1", name: "Inbox", slug: "inbox", position: 0, repoIds: [1, 2] },
      ...
      expect(ids(yield* store.listReposForSearch(LOGIN, { groups: ["inbox", "tools"] }))).toEqual([1, 2, 3]);
  ...
  it.effect("counts stats and caps readme text at 64 KB", () =>
    Effect.gen(function* () {
      ...
      const rows = yield* sql<{ len: number }>`SELECT length(readme_text) AS len FROM repos WHERE id = 2`;
  ...
  it.effect("replaces and lists groups, and maps repos to group slugs", () =>
    Effect.gen(function* () {
      ...
      const inbox: Group = { id: "UL_1", name: "Inbox", slug: "inbox", position: 0, repoIds: [1, 2] };
      const tools: Group = { id: "UL_2", name: "Tools", slug: "tools", position: 1, repoIds: [2, 3] };
      ...
      const cli: Group = { id: "UL_3", name: "CLI", slug: "cli", position: 5, repoIds: [3] };
      ...
      const other: Group = { id: "UL_9", name: "Other", slug: "other", position: 0, repoIds: [1] };
  ...
  it.effect("bulk-upserts repo pages, attributing each row to its listing page", () =>
    Effect.gen(function* () {
      ...
      expect(
        (yield* store.getStarPageRows(LOGIN)).find((row) => row.repoId === 1)?.starPage
      ).toBe(1);
      yield* store.upsertRepoBatch(LOGIN, [repos[0]!], 2);
      expect((yield* store.getStarPageRows(LOGIN)).find((row) => row.repoId === 1)?.starPage).toBe(2);

      // Full columns round-trip, and a re-list without starredAt preserves it.
      yield* store.upsertRepoBatch(LOGIN, [makeRepo(1, "owner/bulk-1", { stars: 42, language: "Rust" })]);
      const [repo] = yield* store.getRepos(LOGIN, [1]);
  ...
  it.effect("reads README texts and planner states for the sync workflow", () =>
    Effect.gen(function* () {
      ...
      yield* store.putReadme(2, { text: null, hash: null, state: "missing", checkedAt: null });
