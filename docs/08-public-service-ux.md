...
| ---------------------- | ------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------- |
| never indexed          | profile resolves, no rows for login         | "Index @{login}'s stars" · "Metadata search in ~10 seconds. Semantic search fills in over the next few minutes — no account needed." (keyword-only deployments say "Indexing reads the public star list first; search works as soon as it lands.") | Index & search               |
...
- **Semantic coverage** — repos embedded / **semantic-window size** (the newest 1,500 repos on free, i.e. `min(star count, 1,500)`, [14 §3.6](14-abuse-protection.md)); grows only in Tier 1. Only exists when the deployment runs semantic search (`STARWATCH_SEMANTIC_SEARCH=1`, [19](19-implementation-status.md#semantic-search-is-optional)): a keyword-only deployment shows neither the counter nor the bar.
