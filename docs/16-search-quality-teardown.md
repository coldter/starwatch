...
**Transfers:** typo/fuzzy tolerance as a baseline expectation; topic/category as *searchable and rankable* fields, not just filters; facet counts + removable chips; a capped, thresholded "similar projects" rail; scores/freshness as small priors (never the primary order); a one-word head term should open a *cluster of intents*, which they achieve with curated alternatives — starwatch must achieve it with topics + semantics.
