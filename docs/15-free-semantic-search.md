...
3,448 docs ÷ 32 per AI call = **108 calls/user** (1,500-repo window: ~47 calls), at/over the **50-subrequest/invocation** Free limit → indexing must span invocations. Both Workflows (Free: 1,024 steps per instance, 10 ms CPU each, 50 subrequests each) and Queues (Free: 10k ops/day) are available; Workflows per-user with ~3 embedding batches (≤ 32 calls each) is the direct mapping to [10 §7](10-multitenant-architecture.md). Content-addressed dedupe ([10 §2.6](10-multitenant-architecture.md)) still applies: shared repos are embedded once, users copy pointers, so the real token cost tracks the _unique_ corpus.
...
- **Routing rules** (refine [07 §7.4](07-search-contract.md)): single token / punctuation / camelCase / name match → `keyword` (no AI call). Multi-token descriptive with no filters → whole-index `hybrid` (semantic ranks come from that pass, 2.3 ms). Descriptive **with** selective filters (language/topic/star range) → filtered gather-dot: resolve `repo_id`s in D1 first, score only those (0.4 ms/1,000). Long noisy queries → the same whole-index pass. `similar` → direct index lookup plus the pass.
...
- **Batching plan** (per user, standard): 108 AI calls = 3 Workflow batches of 36 calls (1,500-repo window: ~47 calls = 2 batches, blob ~3 MB); each call returns 32 × 512d f32 ≈ 64 KB, so one batch holds ~2.3 MB in memory and appends into the blob buffer incrementally; the blob (7 MB) fits the 128 MB isolate alongside the pass. Retry a failed batch once (250 ms backoff), then mark the repo `embed_state='error'` and continue — never fail the whole user for one repo ([03 §1.3](03-sync-and-limits.md) semantics).
