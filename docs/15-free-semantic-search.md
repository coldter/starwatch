...
> Scope: the **hard requirement** is zero spend — Workers **Free** plan only (100k req/day, **10 ms CPU**/request, 128 MB/isolate, 50 subrequests) plus the free allowances of D1 (5 GB total, 500 MB/DB), R2 (10 GB, 1M Class A + 10M Class B ops/mo), Workers AI (10k neurons/day), Cache API and KV. This doc **supersedes the Vectorize semantic leg** in [01 §3.2](01-search-and-index.md), [07 §4.3](07-search-contract.md) and [10 §3](10-multitenant-architecture.md); [07](07-search-contract.md) still owns relevance/latency targets and [12](12-hardening.md) owns degraded-mode behavior. It changes *where vectors live and how they are searched*, not what results must look like.
...
- **One vector per (user, repo)** — 3,448 typical, hard-capped at 5,000 by [12 §1.2](12-hardening.md) `MAX_LIST_PAGES`. No chunk vectors on the free tier: 23k vectors/user costs ~6.3× the embedding tokens (§5) and forces either Vectorize or unacceptable storage/CPU.
...
**Vectors/user: 3,448 (repo-level) vs 23,552 (chunk-level, not on Free). D1/user: <2 KB — pointer row (`vector_blobs`) + `user_semantic` state; no vectors. Cache API/user: one cached object per colo, free, 512 MB/object. R2/user: the table above plus ~7.5 KB/repo of shared README text.**
...
1024d f32 (5.6 ms measured) derates to ~14 ms — **not viable on Free**. 128–256d, or int8 storage for cold tiers, are the levers. The default free-tier search is therefore **hybrid-first** (§4) with the full scan reserved for `semantic`/`similar`.

### 2.5 Indexing path under Free limits

3,448 docs ÷ 32 per AI call = **108 calls/user**, over the **50-subrequest/invocation** Free limit → indexing must span invocations. Both Workflows (Free: 1,024 steps, 10 ms CPU/step, 50 subrequests/step) and Queues (Free: 10k ops/day) are available; Workflows per-user with ~3 embed steps (≤ 32 calls each) is the direct mapping to [10 §7](10-multitenant-architecture.md). Content-addressed dedupe ([10 §2.6](10-multitenant-architecture.md)) still applies: shared repos are embedded once, users copy pointers, so the real token cost tracks the *unique* corpus.

Per user, standard profile: 1 listing sync + 108 batched embed calls across 3 Workflow steps + 1 blob `put` + a handful of D1 writes. Against Free allowances that is 3–4 Worker invocations, well inside 100k req/day and 10k Queue ops/day even at 5 new users/day (~2,400 Workflow step invocations/month).
...
- **Batching**: qwen3 max **32 texts/request** (verified schema); bge-m3/embeddinggemma accept up to 100 ⚠️. 32-text batches mean 108 AI calls per user; Workers AI rate limit is 3,000 req/min — never the constraint.
- **Batching plan** (per user, standard): 108 AI calls = 3 Workflow steps of 36 calls; each call returns 32 × 512d f32 ≈ 64 KB, so one step holds ~2.3 MB in memory and appends into the blob buffer incrementally; the blob (7 MB) fits the 128 MB isolate alongside the scan. Retry a failed batch once (250 ms backoff), then mark the repo `embed_state='error'` and continue — never fail the whole user for one repo ([03 §1.3](03-sync-and-limits.md) semantics).
...
- **Admission**: default **5 new full indexes/day** (standard profile, leaving ~35% headroom for query embeds, churn, retries, MRL experiments); raise toward ~7 if measured cache hit rate on shared repos > 40%. Steady-state churn (~2%/mo of each user's repos) is ~0.024M tokens/user/mo (≈26 neurons) — negligible.
- Count tokens in the `cost_ledger` ([12 §4.2](12-hardening.md)); pause new indexing (not search) when the day's ledger crosses 8,000 neurons.
...
|---|---|---|---|---|---|
| day 1 (MVP) | repo-level | **512 f32** | 7.1 MB | full scan ≤5k (2.3 ms) + candidate rerank | cohort of 100–500 users; expand while R2 < 40% and derating measured |
...
| P1 — semantic behind a flag | `vector_blobs` + Cache API + 512d f32, repo-level, ~5 users/day admission | Q4 R@10 ≥ 0.80, p95 < 500 ms, `exceededCpu` < 0.5%, `cpu_state` wired |
...
| **R2 blobs + in-Worker kNN (this doc)** | ✅ | ✅ 512/256d f32 | ~1.1k users @8 GB | good (MRL ⚠️) | medium | **clean (no re-embed)** |
...
- [12](12-hardening.md): §3.1 budget semantics (CPU-bound), §4.1 global cap becomes an R2/CPU admission cap, degraded reason `cpu`.
