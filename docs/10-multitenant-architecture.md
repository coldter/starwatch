...
The Workers Rate Limiting binding is per-location with only 10 s/60 s windows — unusable as the shared 5,000/h governor. A single DO is safe: state is reconstructed from storage after eviction, and only one object ever owns the lease state. Queues (1M free ops/mo, 15-min consumer wall time) handle dispatch if cron plus Workflow-create throughput becomes the bottleneck (creation limits: 300/s per account, 100/s per Workflow).
