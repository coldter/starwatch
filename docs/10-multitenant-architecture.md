...
> Companions: [00](00-requirements.md) · [01](01-search-and-index.md) (search engine) · [02](02-stack-and-pipeline.md) (pipeline) · [03](03-sync-and-limits.md) (GitHub quota) · [04](04-groups.md) · [07](07-search-contract.md) (contract). This doc owns: sharing model, multi-tenant storage, sharding, eviction, fairness, and cost. ⚠️ marks items to re-verify at implementation time.
...
| Vectors | Vectorize namespace | **yes** (one copy per user; evictable) |
...
| Verdict | ✅ **v1** | ❌ | only as a cold-tier "lite lexical" fallback (v2) |
...
| **Total** | **~35–65 MB** | contentless FTS would land near the low end (⚠️ §2.5) |
...
| Layer | Control |
|---|---|
| Request | Per-IP rate limit (Workers Rate Limiting binding, per-location) + a global D1/DO counter for cross-location accuracy; Turnstile on the WebUI sync form (v1.1); optional `sw_` bearer for CLI ([05 §6.2](05-cli.md)) |
| Identity | Per-IP/day unique usernames (20) and sync requests (50); max total indexed users in Worker config; login must exist and have ≤50k public stars |
| Admission | `sync_jobs` dedupe; new-user admission ≤20/h, ≤200/day; queue overflow returns `429 Retry-After`, not a crash; per-user README/chunk caps, sync ≤ once/6 h, semantic builds only for hot users |
...
3. **Semantic:** lazy namespace per user on one Vectorize index; deterministic vector IDs; metadata indexes created before first upsert; lexical fallback wherever a namespace is absent.
4. **Sync:** cron dispatcher + per-user Workflows + `GithubGovernor` DO (token bucket, leases, priorities, dedupe); lazy on-visit sync for cold users.
5. **Ops:** TTL/LRU eviction job, R2 GC, quota counters, budget kill switch, `usage` dashboards for the Vectorize billing ambiguity (§3.2).
...
1. **Namespace economics + query billing** — at ≥10k active users, does per-user storage still beat a global index, and what do real usage dashboards say about queried dims per namespace query (§3.2)? Both must be answered before semantic-by-default ships.
...
5. **Public star visibility** — `/users/{login}/starred` returns public stars only; private stars are simply absent. Confirm the service messaging and the `star_count` sanity check ([03 §3](03-sync-and-limits.md)).
