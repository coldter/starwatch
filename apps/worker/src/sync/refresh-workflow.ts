...
import { decodeVectors, vectorIdsKey, vectorBlobBinKey, vectorMergeBaseKey, vectorPartBaseKey } from "../adapters/vector-bucket.ts";
...
const readPart = (
  deps: SyncDepsShape,
  base: string
): Effect.Effect<{ readonly ids: ReadonlyArray<number>; readonly vectors: ReadonlyArray<Float32Array> } | null> =>
  Effect.gen(function* () {
    const bytes = yield* deps.vectorFiles.getBytes(`${base}.bin`).pipe(Effect.orDie);
    const ids = yield* deps.vectorFiles.getIds(`${base}.ids.json`).pipe(Effect.orDie);
...
const mergeParts = (
  ...
  Effect.gen(function* () {
    ...
    for (const base of bases) {
      const part = yield* readPart(deps, base);
      if (part === null) continue;
      vectors.push(...part.vectors);
      ids.push(...part.ids);
    }
    const out = vectorMergeBaseKey(login, round, index);
    yield* deps.vectorFiles.putPart(out, vectors, ids).pipe(Effect.orDie);
    return out;
  });
...
const makeRefreshBody = (deps: SyncDepsShape) =>
  Effect.fn("StarRefreshWorkflow.body")(function* (input: StarRefreshInput) {
    ...
    for (let index = 0; index < plan.batches.length; index++) {
      ...
      const outcome = yield* Cloudflare.Workflows.task(
        ...
        Effect.gen(function* () {
          ...
          yield* deps.vectorFiles.putPart(partBase, vectors, dirty).pipe(Effect.orDie);
      ...
      if (!outcome.ok) {
        ...
        return { ok: false, semanticDocs: 0, embedded: embeddedTotal };
      }
    ...
    while (current.length > MERGE_FAN_IN) {
      ...
      for (let index = 0; index < groups.length; index++) {
        const merged = yield* Cloudflare.Workflows.task(
          `merge-${round}-${index}`,
          mergeParts(deps, login, round, index, groups[index] ?? [])
        );
        next.push(merged);
      }
    ...
    const finalize = yield* Cloudflare.Workflows.task(
      ...
      Effect.gen(function* () {
        ...
        yield* deps.vectorFiles.deleteMany(scratch).pipe(Effect.orDie);
    ...
    return { ok: finalize.ok, semanticDocs: finalize.semanticDocs, embedded: finalize.embedded };
  });
...
export class StarRefreshWorkflow extends Cloudflare.Workflow<StarRefreshWorkflow>()(
  ...
  Effect.gen(function* () {
    ...
    const body = makeRefreshBody(deps);
    ...
    return Effect.fn(function* (input: StarRefreshInput) {
      ...
      return { ok: false, semanticDocs: 0, embedded: 0 };
    });
  })
) {}
